...
void loop() {
  ...
  if (((abs(err_lk) < 0.02) && (abs(err_rk) < 0.02)) && ((abs(err_lh) < 0.02) && (abs(err_rh) < 0.02))) {
  ...
  if (corr_lh > 50) corr_lh = 50;
  if (corr_lh < -50) corr_lh = -50;
  if (corr_rh > 50) corr_rh = 50;
  if (corr_rh < -50) corr_rh = -50;

  if (corr_lk > 50) corr_lk = 50;
  if (corr_lk < -50) corr_lk = -50;
  if (corr_rk > 50) corr_rk = 50;
  if (corr_rk < -50) corr_rk = -50;
  ...
  if (err_lk > 0.01) {
    ...
    analogWrite(left_knee_pwm, abs(pwm_lk));
  }
  else if ( err_lk < -0.01) {
    pwm_lk = -30 + corr_lk;
  ...
  if (err_rk > 0.01)
  {
    ...
    analogWrite(right_knee_pwm, abs(pwm_rk));
  }
  else if (err_rk < -0.01)
  {
    pwm_rk = -30 + corr_rk;
  ...
  if (err_lh > 0.01)
  {
    ...
    analogWrite(left_hip_pwm, abs(pwm_lh));
  }
  ...
  if (err_rh > 0.01)
  {
    ...
    analogWrite(right_hip_pwm, abs(pwm_rh));
  }
  else if (err_rh < -0.01)
  {
    pwm_rh = -40 + corr_rh;
